...
class TranslatorTest {
    ...
    fun `the root translator does not have a prefix`() {
        val translator = TranslatorBuilder.create(Locale.FRANCE).build()
    ...
    fun `section prepend the section prefix before the key`() {
        val translator = TranslatorBuilder.create(Locale.FRANCE)
    ...
    fun `translateOrNull returns null if the translation doesnt exist`() {
        val translator = TranslatorBuilder.create(Locale.ENGLISH).build()
    ...
    fun `toString contains the prefix`() {
        val translator = TranslatorBuilder.create(Locale.ENGLISH).build().section("foo")
    ...
    fun `tn overloads coverage`() {
        val t = TranslatorBuilder.create(Locale.ENGLISH)
            .addMap(Locale.ENGLISH, mapOf("key" to "value"))
            .build()
        assertDoesNotThrow {
            t.tn("key")
