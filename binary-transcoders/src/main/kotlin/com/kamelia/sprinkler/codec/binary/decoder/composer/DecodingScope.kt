...
sealed interface DecodingScope<E> {
    ...
    /**
     * Executes the given [block] only one time. The result of the block is cached and returned on subsequent calls.
     *
     * @param key the key to use to cache the result
     * @param block the block to execute
     * @return the result of the block
     * @param T the type of the object to decode
     */
    fun <T> once(key: Any, block: () -> T): T

    /**
     * Executes the given [block] only one time per object. The result of the block is cached and returned on
     * subsequent calls.
     *
     * This method returns a [ReadOnlyProperty] and therefore is intended to be used as a delegate.
     *
     * @param block the block to execute
     * @return a [ReadOnlyProperty] that returns the result of the block
     * @param T the type of the object to decode
     */
    @HideFromJava
    fun <V, T> once(block: () -> T): ReadOnlyProperty<V, T> = ReadOnlyProperty { owner, property ->
        once(owner to property, block)
    }
